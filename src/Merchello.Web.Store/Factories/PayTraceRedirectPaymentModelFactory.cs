namespace Merchello.Web.Store.Factories
{
	...
	public class PayTraceRedirectPaymentModelFactory<TPaymentModel> : CheckoutPaymentModelFactory<TPaymentModel>
		where TPaymentModel : PayTraceRedirectPaymentModel, new()
	{
		public PayTraceRedirectPaymentModelFactory()
		{
		...
		private void SetBillingAddress(TPaymentModel model, ICustomerBase customer)
		{
			...
			if (ba == null)
			{
				LogHelper.Error(typeof(PayTraceRedirectPaymentModelFactory<TPaymentModel>), "Could not retrieve Billing Address for PayTrace Redirect payment. ", new System.Exception("Could not retrieve Billing Address for customer " + customer.Key + " order number " + model.OrderNumber + " at PayTrace Redirect payment step. "));
			}
			else
			{
